...
export function EditTrainingDialog({ open, onOpenChange, training, onTrainingUpdated }: EditTrainingDialogProps) {
  const [title, setTitle] = useState(training.title)
  const [type, setType] = useState(training.type)
  const [instructor, setInstructor] = useState(training.instructor)
  const [startDate, setStartDate] = useState<Date | null>(training.startDate ? new Date(training.startDate) : null)
  const [endDate, setEndDate] = useState<Date | null>(training.endDate ? new Date(training.endDate) : null)
  const [maxCapacity, setMaxCapacity] = useState(training.maxCapacity.toString())
  const [description, setDescription] = useState(training.description)
  const [status, setStatus] = useState(training.status)

  const [isSkillsDialogOpen, setIsSkillsDialogOpen] = useState(false)
  const [selectedSkills, setSelectedSkills] = useState<Skill[]>(training.skills || [])
  const [skillSearchTerm, setSkillSearchTerm] = useState("")

  const [isEvaluationsDialogOpen, setIsEvaluationsDialogOpen] = useState(false)
  const [selectedEvaluations, setSelectedEvaluations] = useState<Evaluation[]>(training.evaluations || [])
  ...
  useEffect(() => {
    ...
    setStartDate(training.startDate ? new Date(training.startDate) : null)
    setEndDate(training.endDate ? new Date(training.endDate) : null)
    setMaxCapacity(training.maxCapacity.toString())
    setDescription(training.description)
    setStatus(training.status)
    setSelectedSkills(training.skills || [])
    setSelectedEvaluations(training.evaluations || [])
  }, [training])
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Modifiez les détails de la formation</DialogDescription>
          </DialogHeader>
